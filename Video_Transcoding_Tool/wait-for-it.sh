...
echo_msg() { if [[ $QUIET -ne 1 ]]; then echo "$@" 1>&2; fi }
...
wait_for()
{
    if [[ $TIMEOUT -gt 0 ]]; then
        echo_msg "$SCRIPT_NAME: waiting $TIMEOUT seconds for $URI"
    else
        echo_msg "$SCRIPT_NAME: waiting for $URI without a timeout"
    fi
    ...
    do
        ...
        if [[ $OUTCOME -eq 0 ]]; then
            WAIT_END_TS=$(date +%s)
            echo_msg "$SCRIPT_NAME: $URI is alive after $((WAIT_END_TS - WAIT_START_TS)) seconds"
            break
        fi
        sleep 1
    done
...
wait_for_wrapper()
{
    ...
    if [[ $OUTCOME -ne 0 ]]; then
        echo_msg "$SCRIPT_NAME: timeout occurred after waiting $TIMEOUT seconds for $URI"
    fi
    return $OUTCOME
}
...
validate_uri() 
{
    ...
    if [[ $curl_exit_code -eq 3 ]]; then # exit code 3 indicates an invalid URI
        echo_msg "Error: you need to provide a VALID URI to test."
        usage
    fi
}
...
if [[ "$URI" == "" ]]; then
    echo_msg "Error: you need to provide a URI to test."
    usage
fi
validate_uri
...
if [[ $COMMAND != "" ]]; then
    if [[ $OUTCOME -ne 0 && $STRICT -eq 1 ]]; then
        echo_msg "$SCRIPT_NAME: strict mode, refusing to execute subprocess"
        exit $OUTCOME
    fi
    exec "${COMMAND[@]}"
else
